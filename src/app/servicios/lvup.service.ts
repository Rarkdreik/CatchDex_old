...
export class LvupService {
  ...
  public calcularBarraExp(barraExp: any, entidad: PokemonInterface | Entrenador) {
  ...
  public obtenerExpEntrenador(entrenador: Entrenador, pokeSalvaje: PokemonInterface) {
    let contenedor = (entrenador.nivel + 1) * (entrenador.nivel + 1) * (entrenador.nivel + 1);
    let expGanada = 0;

    switch (pokeSalvaje.ball) {
      case 'pokeball':
        expGanada = this.getExpRelativa(entrenador, contenedor, pokeSalvaje, 20, 18, 16, 10);
        break;
      case 'superball':
        expGanada = this.getExpRelativa(entrenador, contenedor, pokeSalvaje, 14, 12, 10, 8);
        break;
      case 'ultraball':
        expGanada = this.getExpRelativa(entrenador, contenedor, pokeSalvaje, 8, 4, 2, 1);
        break;
      case 'masterball':
        expGanada = this.getExpRelativa(entrenador, contenedor, pokeSalvaje, 1, 1, 1, 1);
        break;
      default:
        expGanada = this.getExpRelativa(entrenador, contenedor, pokeSalvaje, 15, 13, 11, 9);
        break;
    }

    entrenador.exp += expGanada;

    while (entrenador.exp >= contenedor) {
      entrenador.nivel += 1;
      entrenador.exp -= contenedor;
      contenedor = (entrenador.nivel + 1) * (entrenador.nivel + 1) * (entrenador.nivel + 1)
    }
    return entrenador;
  }
  ...
  /**, y la devuelve
   */
  private getExpRelativa(entrenador: Entrenador, contenedor: number, pokeSalvaje: PokemonInterface, expEnorme: number, expGrande: number, expNormal: number, expPequeño: number) {
    let expGanada: number = 0;
    if (pokeSalvaje.nivel > entrenador.nivel) {
  ...
  public nivelRango5(master: Entrenador): number {
  ...
  public setCaracteristicas(entrenador: Entrenador ,pokemon: PokemonInterface) {
    let aux: number; let min: number; let max: number;

    if ((entrenador.nivel - 5) > 0) { min = (entrenador.nivel - 5); } else { min = 1; }
    max = entrenador.nivel + 5;
